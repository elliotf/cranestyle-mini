...
module belt_clamp() {
  y_belt_clamp_width = 10; // walter has 8
  y_belt_clamp_length = 20; // walter has 22
        ...
        translate([y_belt_clamp_width/2,belt_thickness,0]) {
          ...
          intersection() {
            ...
            translate([-rounding_teeth_od/2,rounding_teeth_od/2,0]) {
              difference() {
                accurate_circle(rounding_teeth_od+belt_thickness*2+0.1,resolution);
  ...
  module holes() {
    for(y=[front,rear]) {
      translate([0,y*y_belt_clamp_length/4,0]) {
        hole(m3_loose_diam,y_belt_clamp_height+1,resolution);
      }
    }
